...
class Database:
    ...
    def insert_new_raffle_ticket(self, username: str, nameJersey: str, team: str, jerseyNumber: int, size: str, gender: str) -> None:
        """
        Inserts a new raffle ticket into the database.

        args:
            - username: Username of user.
            - nameJersey: Name for user jersey.
            - team: Team name for jersey of user choice.
            - jerseyNumber: User choice of number for jersey.
            - size: User size choice of jersey.
            - gender: User gender. 

        returns:
            - None
        """
        self.cursor.execute("INSERT INTO raffle (username, name_jersey, team, jersey_number, size, gender) VALUES (?, ?, ?, ?, ?, ?)", (username, nameJersey, team, jerseyNumber, size, gender))
        self.connection.commit()

    def current_raffle_count(self) -> int:
        """
        Fetches current raffle count.

        args:
            - None.

        returns:
            - Count of raffle tickets.
        """
    ...
    def add_to_wishlist(self, username: str, product_id: int) -> None:
        """
        ...
        Returns:
            None
        """
        jersey_info=self.get_item_name_by_id(int(product_id))["info"]
        print(jersey_info)
        jersey_image_url=self.get_item_image_url_by_id(int(product_id))["image_url"]
        print(jersey_image_url)
        self.cursor.execute("INSERT INTO wishlist (username, jersey_info, jersey_image_url) VALUES (?, ?, ?)", (username, jersey_info, jersey_image_url))
        self.connection.commit()
    ...
    def remove_from_wishlist(self, jersey_id):
        ...
        Parameters:
            jersey_id (int): The ID or name of the jersey to remove.

        Returns:
            None
        """
        self.cursor.execute("DELETE FROM wishlist WHERE jersey_id = ?", (jersey_id,))
        self.connection.commit()
    ...
    def get_all_jersey_ids(self):
        """
        Gets all jersey_id in the wishlist database.

        args:
            - None

        returns:
            - List of all jersey_id in the database.
        """
        self.cursor.execute("SELECT jersey_id FROM wishlist")
        return self.cursor.fetchall()

    def get_all_jersey_info(self):
        """
        Gets all jersey_id in the wishlist database.
        ...
        returns:
            - List of all jersey_id in the database.
        """
        self.cursor.execute("SELECT username, jersey_id, jersey_team, jersey_size, jersey_price FROM wishlist")
        return self.cursor.fetchall()
    
    def get_jersey_id(self, jersey_id: int):
        """
        Gets jersey_id in the wishlist database.

        args:
            - None

        returns:
            - List the jersey_id in the database.
        """
        self.cursor.execute("SELECT jersey_id FROM wishlist WHERE jersey_id = ?", (jersey_id,))
        return self.cursor.fetchone()

    def get_username(self, username: str):
        """
        Gets username in the wishlist database.

        args:
            - None

        returns:
            - List the username in the database.
        """
        self.cursor.execute("SELECT username FROM wishlist WHERE username = ?", (username,))
        return self.cursor.fetchone()
    
    def get_jersey_team(self, jersey_id: int):
        """
        Gets jersey_team in the wishlist database.

        args:
            - None

        returns:
            - List the jersey_team in the database.
        """
        self.cursor.execute("SELECT jersey_team FROM wishlist WHERE jersey_id = ?", (jersey_id,))
        return self.cursor.fetchone()
    
    def get_jersey_size(self, jersey_id: int):
        """
        Gets jersey_size in the wishlist database.

        args:
            - None

        returns:
            - List the jersey_size in the database.
        """
        self.cursor.execute("SELECT jersey_size FROM wishlist WHERE jersey_id = ?", (jersey_id,))
        return self.cursor.fetchone()

    def get_jersey_price(self, jersey_id: int):
        """
        Gets jersey_price in the wishlist database.

        args:
            - None

        returns:
            - List the jersey_price in the database
        """
        self.cursor.execute("SELECT jersey_price FROM wishlist WHERE jersey_id = ?", (jersey_id,))
        return self.cursor.fetchone()
    ...
    def get_item_size_by_id(self, item_id: int):
        """
        Gets the size of an item from the database.

        args:
            - item_id: The id of the item to get.

        returns:
            - The size of the item with the given id.
        """
        self.cursor.execute(
            "SELECT size FROM inventory WHERE id = ?", (item_id,))
        return self.cursor.fetchone()
    ...
    def set_item_size(self, item_id: int, new_size: str):
        """
        Updates the information of an item in the database.
        ...
        """
        self.cursor.execute(
            "UPDATE inventory SET size = ? WHERE id = ?", (new_size, item_id))
        self.connection.commit()
    ...
    def discount_all_items_prices(self, discount: float=0.1):
        """
        Applies discount to user purchase.

        args:
            - discount: percentage off of purchase.

        returns:
            - None
        """
        self.cursor.execute(
            "UPDATE inventory SET price = price-(price*?)", (discount,))
        self.connection.commit()
